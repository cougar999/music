...
var weburl = 'http://localhost/music/'
  ...
  players.on = {
    play: function(player, soundObject) {
      $("#record_bg img").addClass("rotateAlltheTime");
      //$("#record_stylus").removeClass("record_stylus").addClass('record_stylus_playing');
      $("#record_stylus").addClass('record_stylus_playing');
    },
    pause: function(player, soundObject) {
      $("#record_bg img").removeClass("rotateAlltheTime");
      $("#record_stylus").removeClass("record_stylus_playing");
    },
    ...
    whileplaying: function(player, soundObject) {
      if(!$("#record_bg img").hasClass("rotateAlltheTime")){
          $("#record_bg img").addClass("rotateAlltheTime");
      };
      if(!$("#record_stylus").hasClass("record_stylus_playing")){
          $("#record_stylus").addClass('record_stylus_playing');
      };
    },
    /*
  ...
  Player = function(playerNode) {
    ...
    function playLink(link, fromClick) {
      ...
      if (fromClick) {
        ...
        var track = {url:link.getAttribute("href")};
        findUrl(track, function(item){
          soundlink = item[0].url;
          canplay = soundManager.canPlayURL(soundlink);
        });
      ...
      if (canplay) {
        ...
        if (!soundObject) {
          soundObject = makeSound(soundlink);
        }
    ...
    function findUrl(track, callbacks, ignore) {
      if (ignore) { return; }
      $.ajax({
        url: weburl + 'tracks/get-track-url', 
        type: 'POST',
        data: track,
        dataType: 'json',
        async:false,
    ...
    function PlaylistController() {
      ...
      function select(item) {
        ...
        if (item) {
          ...
          if (itemBottom > containerHeight + scrollTop) {
            // bottom-align
            dom.playlist.scrollTop = (itemBottom - containerHeight)*2 + itemPadding;
      ...
      function getURL() {
        ...
        //var track = {url:item.getElementsByTagName('a')[0].href};
        var track = {url:item.getElementsByTagName('a')[0].getAttribute("href")};
    ...
    function handleClick(e) {
      ...
      if (target && target.nodeName) {
        ...
        if (targetNodeName === 'a') {
          ...
          href = target.href;
          var basedcode = target.getAttribute("href");

          var track = {url:basedcode};
          ...
          if (canplay) {
            ...
            if (!utils.css.has(target, playerOptions.excludeClass)) {
              ...
              target.href = soundlink;
              playLink(target, true);

              handled = true;
              target.href = basedcode;
            }
    ...
    actions = {
      ...
      play: function(offsetOrEvent) {
        ...
        if (!href || href.indexOf('#') !== -1) {
          ...
          var code = dom.playlist.getElementsByTagName('a')[0].getAttribute("href")
          var track = {url:code};
      ...
      menu: function(ignoreToggle) {
        ...
        //document.getElementsByTagName('footer')[0].style.marginBottom = (isOpen ? dom.playlistContainer.scrollHeight+45 : 0) + 'px';

      },
      menuOpen: function() {
        var isOpen = dom.playlistContainer.style.height;
        if (isOpen !== 0 && isOpen !== null && isOpen !== '') {
          return true;
        } else {
          return false;
        }
